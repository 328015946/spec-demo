...
def _parse_hook_input(input_data: dict) -> tuple[str, str, dict]:
    ...
    - Gemini CLI: tool_name=invoke_agent, tool_input.agent_name
    ...
    if tool_name.lower() in ("task", "agent", "subagent", "invoke_agent", "invokeagent"):
        return (
    ...
    # Gemini CLI (legacy/matcher-based): BeforeTool where tool_name IS the agent name
